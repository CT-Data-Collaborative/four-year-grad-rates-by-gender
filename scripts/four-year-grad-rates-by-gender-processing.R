...
##################################################################
#
# Processing Script for Four-Year-Grad-Rates-by-Gender
...
backfill_years <- expand.grid(
  ...
  `Gender` = unique(four_yr_gr$`Gender`),
  `Year` = years
)
...
complete_four_yr_gr <- complete_four_yr_gr[!with(complete_four_yr_gr, is.na(complete_four_yr_gr$`Gender`)),]
...
test <- complete_four_yr_gr_long[,c("District", "Year", "Gender", "Variable")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_four_yr_gr_long,
  file.path(path_to_top_level, "data", "four_year_grad_rate_by_gender_2011-2019.csv"),
  sep = ",",
  row.names = F
)
